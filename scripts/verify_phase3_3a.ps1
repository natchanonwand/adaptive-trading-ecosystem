...
& uv run --locked --extra discovery pytest -q -p no:cacheprovider --junitxml=test-results/phase3_3a.xml
if ($LASTEXITCODE -ne 0) { throw 'Tests failed' }
# The full suite above includes tests/simulation/test_scope.py and prior safety checks.
...
Write-Output 'PASS: Phase 3.3A synthetic kernel and deterministic fixtures, prior-phase tests, fresh PostgreSQL and scope/safety checks'
